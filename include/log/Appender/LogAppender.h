...
namespace CLSN {
    ...
    public:
        ...
        [[nodiscard]] bool recordValid(const LogRecord &record) const noexcept {
            return (static_cast<short >(record.getLevel()) >= static_cast<short >(appenderLevel))
                   && valid();
        }
    ...
    public:
        ...
        [[nodiscard]] bool valid() const noexcept override {
            return static_cast<bool>(formatter);
        }
    };
    ...
    public:
        FileLogAppender(const std::string &filename, const std::string &format,
                        LogLevel level = LogLevel::Debug) :
                LogAppender(format, level) {
            reSetFileName(filename);
        }

        explicit FileLogAppender(const std::string &filename, LogLevel level = LogLevel::Debug) :
                LogAppender(level) {
            reSetFileName(filename);
        }
        ...
        bool valid() const noexcept override {
            return ofStream.is_open() && static_cast<bool>(formatter);
        }
