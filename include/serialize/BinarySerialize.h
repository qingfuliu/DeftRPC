...
namespace CLSN {
    ...
    public:
        ...
        void OutPut(const void *data, size_t size) {
            std::streamsize n =
                    os.rdbuf()->sputn(static_cast<const char *>(data), static_cast<std::streamsize>(size));
            if (size != n) {
                throw "";
            }
        }
    ...
    class FileBinarySerialize : public BinarySerialize {
        explicit FileBinarySerialize(const std::string &fileName) :
                BinarySerialize(of) {
    ...
    public:
        explicit BinaryDeSerialize(std::istream
                                   &inPutStream) noexcept: Base(this),
                                                           is(inPutStream) {}
        ...
        void Input(void *data, size_t size) {
            std::streamsize n =
                    is.rdbuf()->sgetn(static_cast<char *>(data), static_cast<std::streamsize>(size));
            if (size != n) {
                throw "";
            }
        }
