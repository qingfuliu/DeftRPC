...
namespace CLSN {
    ...
    std::enable_if_t<!has_load_and_construct_v<Sr, T>>
    DEFTRPC_SERIALIZE_OUTPUT_FUNCNAME(Sr &sr, const std::shared_ptr<T> &ptr) noexcept {
    ...
    template<typename Sr, typename T>
    std::enable_if_t<!has_load_and_construct_v<Sr, T>>
    DEFTRPC_DESERIALIZE_INPUT_FUNCNAME(Sr &sr, std::shared_ptr<T> &ptr)
    noexcept(noexcept(access::construct<T>())) {
        IdType id = 0;
        sr(make_size_tag(id));
        if (static_cast<bool>(id & PtrIdMask)) {
    ...
    std::enable_if_t<has_load_and_construct_v<Sr, T>>
    DEFTRPC_DESERIALIZE_INPUT_FUNCNAME(Sr &sr, std::shared_ptr<T> &ptr)
    noexcept(ConstructNavigation(std::declval<Sr>(),
    ...
    template<typename Sr, typename T>
    std::enable_if_t<!has_load_and_construct_v<Sr, T>>
    DEFTRPC_DESERIALIZE_INPUT_FUNCNAME(Sr &sr, std::unique_ptr<T> &ptr)
    noexcept(noexcept(access::construct<T>())) {
        short id;
        sr(make_size_tag(id));
    ...
    template<typename Sr, typename T>
    void DEFTRPC_SERIALIZE_OUTPUT_FUNCNAME(Sr &sr, const std::weak_ptr<T> &ptr)
    noexcept(DEFTRPC_SERIALIZE_OUTPUT_FUNCNAME(std::declval<Sr &>(), std::declval<std::shared_ptr<T> &>())) {
    ...
    template<typename Sr, typename T>
    void DEFTRPC_DESERIALIZE_INPUT_FUNCNAME(Sr &sr, std::weak_ptr<T> &ptr)
    noexcept(DEFTRPC_DESERIALIZE_INPUT_FUNCNAME(std::declval<Sr &>(), std::declval<std::shared_ptr<T> &>())) {
        auto sharedPtr = ptr.lock();
