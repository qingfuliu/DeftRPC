...
namespace CLSN {
    ...
    inline std::enable_if_t<ConditionalAnd_v<std::is_arithmetic<T>, is_binary_serialize<Sr> >,
            void>
    DEFTRPC_SERIALIZE_OUTPUT_FUNCNAME(Sr &sr, const std::vector<T, A> &vec) noexcept {
        sr(make_size_tag(static_cast<size_t>(vec.size())));
        if (!vec.empty()) {
            sr.OutPut(static_cast<const void *>(vec.data()),
                      vec.size() * sizeof(T));
        }
    }

    template<typename Sr, typename T, typename A>
    inline std::enable_if_t<ConditionalAnd_v<std::is_arithmetic<T>, is_binary_deserialize<Sr> >,
            void>
    ...
    template<typename Sr, typename T, typename A>
    inline std::enable_if_t<!ConditionalOr_v<is_binary_serialize<Sr>, std::is_same<bool, T> >,
            void>
    DEFTRPC_SERIALIZE_OUTPUT_FUNCNAME(Sr &sr, const std::vector<T, A> &vec) noexcept {
    ...
    template<typename Sr, typename T, typename A>
    inline std::enable_if_t<!ConditionalOr_v<is_binary_deserialize<Sr>, std::is_same<bool, T> >,
            void>
    DEFTRPC_DESERIALIZE_INPUT_FUNCNAME(Sr &sr, std::vector<T, A> &vec) noexcept {
        int size = 0;
        sr(make_size_tag(size));
