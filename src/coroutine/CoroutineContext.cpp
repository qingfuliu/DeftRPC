...
namespace CLSN {
    ...
    void CoroutineContext::makeCtx() noexcept {
        ...
        *reinterpret_cast<void**>(bp+sizeof(void*))=
    ...
    void CoroutineContext::saveStackToSavedStack() {
        ...
        mem.validSize = static_cast<size_t>(mem.stack_bp
                                            - static_cast<char *>( ctx.regs[kRSP]));
        if (mem.validSize >= mem.saveStackSize) {
            ...
            if (nullptr != mem.savedStack)
                delete[] reinterpret_cast<char *>(mem.savedStack);
    ...
    void CoroutineContext::loadStackFromSavedStack() {
        sharedMem->LoadCtxFromStack(mem.savedStack, mem.validSize);
    }
