...
class StrFormatterItem : public FormatterItem {
 public:
  ...
  void format(std::string &&temp) noexcept { v.append(temp); }
  ...
  for (; prev <= format.size() - 1 && index != std::string::npos; index = format.find('%', prev)) {
    do {
      if (index >= format.size() - 1) {
        ...
        first->addItem(new StrFormatterItem(std::string(format.begin() + prev, format.begin() + index + 1)));
        break;
      }
...
#define APPEND_ITEM(CON, CLASSNAME, ARG)                                                                   \
  case CON:                                                                                                \
    if (prev < index) {                                                                                    \
      if (timeBegin != -1) {                                                                               \
        prev = timeBegin;                                                                                  \
        first->addItem(new TimeFormatterItem(std::string(format.begin() + prev, format.begin() + index))); \
        timeBegin = -1;                                                                                    \
      } else                                                                                               \
        first->addItem(new StrFormatterItem(std::string(format.begin() + prev, format.begin() + index)));  \
    }                                                                                                      \
    first->addItem(new CLASSNAME(ARG));                                                                    \
    break;
        case 'Y':
        case 'm':
        case 'd':
...
#undef APPEND_ITEM
  ...
  if (timeBegin != -1) {
    first->addItem(new TimeFormatterItem(std::string(format.begin() + timeBegin, format.end())));
