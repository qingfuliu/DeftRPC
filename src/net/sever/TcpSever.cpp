...
namespace CLSN {
    ...
    void TcpSever::CloseConnection(int fd, bool activelyClose) noexcept {
        if (IsInLoopThread()) {
            ...
            if (activelyClose) {
                if (0 != close(fd)) {
                    CLSN_LOG_ERROR << "close socket " << fd << "error when "
                                                               "~connection,error is " << errno;
                }
                it->second->swapIn();
            }
    ...
    void TcpSever::newConnectionArrives(int fd, const Addr &remote) noexcept {
        auto it = connections.emplace(fd, CreateCoroutine([fd, remote]() {
    ...
    void TcpSever::closeAllConnection() noexcept {
        ...
        std::for_each(connections.begin(), connections.end(), [&temp](auto &x) {
