...
RingBuffer::RingBuffer() noexcept:
        ...
        size(0),
        buffer(std::vector<char>(DefaultBufferLen)),
        tempCapacity(0),
        temp(nullptr) {}

uint32_t RingBuffer::GetInt() const noexcept {
    ...
    if (tailLen >= sizeof(uint32_t)) {
        const int *res = reinterpret_cast<const int *>(&(*(buffer.begin() + begin)));
        return *res;
    }
    uint32_t res = 0;
    std::copy(buffer.begin() + begin, buffer.end(), reinterpret_cast<char *>(&res));
    std::copy(buffer.begin(), buffer.begin() + sizeof(int) - tailLen, reinterpret_cast<char *>(&res));
    return ntohl(res);
}
...
int RingBuffer::getWritableCapacity() const noexcept {
    return static_cast<int>( buffer.capacity()) - this->size;
}
...
int RingBuffer::Read(char *buf, int len) noexcept {
    ...
    do {
        ...
        std::copy(buffer.begin() + begin, buffer.begin() +
                                          begin + tailLen, buf);
        if (len > tailLen) {
            std::copy(buffer.begin(), buffer.begin() +
                                      len - tailLen, buf + tailLen);
        }
    } while (0);
    updateAfterRead(len);
    return len;
