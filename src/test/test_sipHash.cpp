...
// I made some changes to the header files and removed a small number of unimportant parts
...
void BM(benchmark::State &state) {
  ...
  const HH_U64 keys[4] = {0x0706050403020100ULL, 0x0F0E0D0C0B0A0908ULL, 0x1716151413121110ULL, 0x1F1E1D1C1B1A1918ULL};
  Hasher hasher(keys);
  for (auto s : state) {
...
void Args(::testing::Benchmark *bm) { bm->DenseRange(1, 16)->Range(32, 100 << 20); }

#define DEFINE_HASHER(hashfn, num_keys)                                                                  \
  struct hashfn##er {                                                                                    \
    hashfn##er(const HH_U64 *k) { memcpy(keys, k, sizeof(keys)); }                                       \
    HH_U64 operator()(const char *input, size_t size) { return highwayhash::hashfn(keys, input, size); } \
