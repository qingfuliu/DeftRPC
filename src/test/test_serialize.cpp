...
//    }
    ...
    [[nodiscard]] std::string to_string() const noexcept {
        return "X:" + std::to_string(x) + " Y:" + std::to_string(y) + " Z:" + std::to_string(z) + " ptr:" +
               std::to_string(*ptr);
    }
...
namespace temp {
    ...
    template<class sr, typename T, typename v=std::enable_if<std::is_same_v<void, decltype(DeSerializeFunc(
    ...
    template<class Sr, typename T>
    struct has_no_member_deserialize_func
            : public decltype(has_no_member_deserialize_func_helper<std::decay_t<Sr>, T>(0)) {
    };


    template<class sr, typename T, typename v= std::enable_if<std::is_same_v<void, decltype(
    LoadAndConstruct(std::declval<sr &>(), std::declval<Construct<std::remove_reference_t<T>> &>()))>>>
    ...
    template<class Sr, typename T> inline constexpr bool has_no_member_load_and_construct_func_v = has_no_member_load_and_construct_func<Sr, T>::value;
...
static void test_binary_serialize(benchmark::State &state) {
    ...
    for (auto _: state) {
        ...
        std::cout << "A" << std::endl;
        std::cout << has_member_serialize_func_v<BinarySerialize,
                A> << std::endl;
        std::cout << has_no_member_serialize_func_v<BinarySerialize,
                A> << std::endl;


        std::cout << has_member_deserialize_func_v<BinarySerialize,
                A &> << std::endl;
        std::cout << has_no_member_deserialize_func<BinarySerialize,
                A &>::value << std::endl;


        std::cout << has_member_load_and_construct_func<BinarySerialize,
                A &>::value << std::endl;
...
//                        decltype(LoadAndConstruct(std::declval<BinarySerialize &>(), std::declval<Construct<A> &>()))>>;
        ...
        std::cout << has_no_member_load_and_construct_func<BinarySerialize,
                A &>::value << std::endl;
        ...
        using p1 = std::enable_if<std::is_same_v<void, decltype(DeSerializeFunc(
                std::declval<BinarySerialize &>(), std::declval<std::vector<int> &>()))>>;

        std::cout << has_member_deserialize_func_v<BinarySerialize,
                std::vector<int>> << std::endl;
        std::cout << has_no_member_deserialize_func<BinarySerialize,
                std::vector<float> &>::value << std::endl;
...
static void test_binary_serialize_vector(benchmark::State &state) {
    for (auto _: state) {
...

        {
            ...
            std::ifstream f("/home/lqf/project/DeftRPC/src/test/binary.txt",
                            std::ios_base::binary);
            if (!f.is_open()) {
                std::cout << "open file failed!" << std::endl;
            }
...
//            A a{1,2,3};
            ...
            BinarySerialize bsr(f);
            bsr(ptr);
            std::cout << "test_binary_serialize_memory_complex: " << ptr->to_string() << " use count: "
                      << ptr->ptr.use_count() << " " << ptr->ptr1.use_count() << std::endl;
            f.close();
        }
